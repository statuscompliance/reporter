...
for(var i = 0; i < metrics.length; i++){
  ...
  for(var j = 0; j < guaranteesData[metrics[i]].of.length; j++){
      var style = {
        ...
        "linkUrl": agreement.context.infrastructure.reporter.slice(0, -7)  + "/evidences/#!/?agreement=" + agreementId + "&guarantee=" + metrics[i] + "&priority=" + guaranteesData[metrics[i]].of[j].scope.priority  + "&period=$__cell_0&value=$__cell&tz=" + timezone,
    ...
    var threshold =  {
      "colorMode": "custom",
      "lineColor": graph.aliasColors[guaranteesData[metrics[i]].of[j].scope.priority],
      "fill": false,
      "line": true,
      "op": getConditionType(guaranteesData[metrics[i]].of[j].objective),
      "value": parseInt(guaranteesData[metrics[i]].of[j].objective.split("=")[1].trim())
    }
