...
exports.dashboardGET = async (req, res, next) => {
  ...
  const agreementRequest = await governify.infrastructure
    .getService("internal.registry")
    .get("/api/v6/agreements/" + agreementId);
  ...
  try {
    ...
    if (dashboardConfig.config && dashboardConfig.config.configDashboard) {
      ...
      dashboardJSON = await customDashboard(
        dashboardConfig,
        agreement,
        dashboardId
      );
    ...
    console.log("Err: ", err);
    logger.error(
      "Error getting dynamic dashboard. agreementId: " +
        agreementId +
        " dashboardId: " +
        dashboardId +
        " - ERROR: " +
...
const customDashboard = async (dashboardConfig, agreement, dashboardId) => {
  // Get the JSON file
  var dashboardJSON = await governify.utils.loadObjectFromFileOrURL(
    dashboardConfig.base
  );
  // Replace all agreement variables specified in the json
  var stringDashboardJSON = utils.textReplaceReferencesFromJSON(
  ...
  dashboardConfig.modifier =
    "src/backend/controllers/v4/dashboards/modifier.js";

  // Check if there is a base modifier
  if (dashboardConfig.modifier && dashboardConfig.modifier !== "") {
    var dashboardBaseModifier = await governify.utils.requireFromFileOrURL(
      dashboardConfig.modifier,
      dashboardConfig.modifier
    );

    // Apply modifier functions of the dashboard
    dashboardJSON = dashboardBaseModifier.modifyJSON(
      dashboardJSON,
      agreement,
      dashboardId
    );
  ...
  if (
    dashboardConfig.modifierPipe &&
    typeof dashboardConfig.modifierPipe === "object"
  ) {
    const modifierPipe = Object.entries(dashboardConfig.modifierPipe).sort(
      (a, b) => a[0] - b[0]
    );
    //Apply each modifier
    for (const [_, modifier] of modifierPipe) {
      var dashboardModifier = await governify.utils.requireFromFileOrURL(
        modifier,
        modifier
      );
      dashboardJSON = dashboardModifier.modifyJSON(
        dashboardJSON,
        agreement,
        dashboardId
      );
...
const configDashboard = async (agreement, dashboardId) => {
  ...
  dashboardJSON = JSON.parse(stringDashboardJSON);
  dashboardJSON = blocksToDashboard.default(
    dashboardJSON,
    agreement,
    dashboardId
  );
